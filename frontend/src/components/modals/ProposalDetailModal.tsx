...
const ProposalDetailModal: React.FC<ProposalDetailModalProps> = ({ isOpen, onClose, proposal }) => {
  ...
  useEffect(() => {
    if (isOpen && proposal) {
      getProposalSignatures(parseInt(proposal.id, 10)).then((result) => {
        setSigners(result);
      });
    }
  ...
  useEffect(() => {
    if (!isOpen) return;
    const t = setTimeout(() => setActiveTab('details'), 0);
  ...
  const handleRemind = async (address: string) => {
    await remindSigner(parseInt(proposal.id, 10), address);
  };

  const handleExport = () => {
    exportSignatures(parseInt(proposal.id, 10));
  };

  const handleRefreshSignatures = async () => {
    const updated = (await getProposalSignatures?.(parseInt(proposal.id, 10))) ?? [];
    setSigners(updated);
  };
  ...
  const flowSteps: FlowStep[] = [
    ...
    {
      label: `Collecting Signatures (${signers.filter((s) => s.signed).length}/${proposal.threshold ?? 3})`,
      status: signers.filter((s) => s.signed).length >= (proposal.threshold ?? 3) ? 'completed' : 'active',
    },
    {
      ...
      <div className="bg-secondary w-full max-w-2xl h-fit max-h-[90vh] flex flex-col rounded-2xl border border-gray-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-6 space-y-6 custom-scrollbar">
          {activeTab === 'details' ? (
            <>
              {proposal.memo != null && (
                <p className="text-sm">
                  <span className="text-gray-400">Memo:</span> <span className="text-white">{proposal.memo}</span>
                </p>
              )}
              {proposal.amount != null && proposal.token != null && (
                <p className="text-sm">
              ...
              {proposal.createdAt != null && (
                <p className="text-sm">
                  <span className="text-gray-400">Created:</span> <span className="text-white">{proposal.createdAt}</span>
                </p>
              )}
                ...
                <div className="flex justify-between items-start relative px-2">
                  {[
                    ...
                    <div key={step.label} className="flex flex-col items-center flex-1 relative z-10">
                      ...
                      <span
                        className={`mt-3 text-[10px] font-bold ${step.active ? 'text-white' : 'text-gray-600'}`}
                      >
                        {step.label}
                      </span>
                      {idx !== arr.length - 1 && (
                        <div className="absolute top-[1.1rem] left-1/2 w-full h-[2px] -z-10 overflow-hidden">
                          <div
                            className={`h-full w-full ${arr[idx + 1].active ? 'bg-accent' : 'bg-gray-800'}`}
                          />
                        </div>
                      )}
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-primary/30 p-4 rounded-xl border border-gray-800/50">
                  ...
                  <div className="flex justify-between items-center bg-black/40 p-2.5 rounded-lg border border-white/5">
                    <code className="text-xs text-gray-300 font-mono truncate mr-3">
                      {proposal.proposer ?? 'GA...7K9L'}
                    </code>
                    <button
                      onClick={() => copyToClipboard(proposal.proposer)}
                      className="text-accent hover:scale-110 active:scale-90 transition-all shrink-0"
                      type="button"
                    >
                      <Copy size={14} />
                    </button>
              ...
              <div className="bg-primary/20 rounded-xl border border-gray-800 overflow-hidden">
                ...
                <div className="divide-y divide-gray-800/50">
                  {[1, 2].map((_, i) => (
                    <div
                      key={i}
                      className="px-4 py-3.5 flex justify-between items-center text-xs hover:bg-white/[0.02] transition-colors"
                    >
        ...
        <div className="p-4 sm:p-6 border-t border-gray-800 bg-secondary/80 shrink-0 backdrop-blur-md">
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              className="flex-1 bg-accent hover:bg-accent/90 text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all hover:scale-[1.01] active:scale-[0.98] shadow-lg shadow-accent/10 text-sm"
              type="button"
            >
              <CheckCircle2 size={18} /> Approve Proposal
            </button>
            <button
              className="flex-1 bg-secondary border border-red-500/20 text-red-500 hover:bg-red-500/10 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.98] text-sm"
              type="button"
            >
              <Ban size={18} /> Reject
            </button>
